...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    // ⭐⭐⭐ CORRECTION ICI : Utiliser points_of_sale_ids au lieu de points_of_sale ⭐⭐⭐
    const pointsOfSaleIds = formData.points_of_sale_ids
      ? formData.points_of_sale_ids.map(id => parseInt(id, 10))
      : [];
    ...
    if (modalType === 'edit' && selectedUser) {
      dataToSend = {
        ...
        points_of_sale_ids: pointsOfSaleIds, // ⭐ CHANGER ICI ⭐
      };
    } else {
      dataToSend = {
        user: {
    ...
    console.log('Payload envoyé:', dataToSend); // Pour débogage
