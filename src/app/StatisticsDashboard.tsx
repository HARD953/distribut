...
const StatisticsDashboard = () => {
  ...
  // Réinitialiser la pagination quand on change d'onglet ou de données
  useEffect(() => {
    setCurrentPage(1);
  }, [activeTab, stats.purchaseDetails]);
  ...
  // Fonctions de pagination pour les détails des achats
  const totalPages = Math.ceil(stats.purchaseDetails.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentPurchaseDetails = stats.purchaseDetails.slice(startIndex, endIndex);
...
const ChartComponent: React.FC<{ 
  ...
  if (!chartData || !chartData.labels || chartData.labels.length === 0) {
    ...
    // Conversion vers le format Chart.js avec typage correct
    const chartJSData: ChartJSData<'bar' | 'line' | 'pie', number[], string> = {
      labels: chartData.labels,
 ...
 // Conversion vers le format Chart.js avec typage correct pour chaque type
          ...
          {topVendors.map((vendor, index) => (
            <div key={vendor.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 transition-colors border border-gray-100">
              ...
              <div className="text-right">
                ...
                <p className="text-xs text-gray-500">Efficacité: {(vendor.performance || 0).toFixed(1)}%</p>
              </div>
            </div>
          ))}
        </div>
                ...
                {stats.vendorStats.map((vendor) => (
                  <tr key={vendor.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-3 px-4 text-right">
                      <div className="flex items-center justify-end">
                        <span className={`text-sm font-medium ${
                          (vendor.performance || 0) >= 80 ? 'text-green-600' :
                          (vendor.performance || 0) >= 60 ? 'text-yellow-600' : 'text-red-600'
                        }`}>
                          {(vendor.performance || 0).toFixed(1)}%
                        </span>
                      </div>
                    </td>
  ...
  const PushcartTab: React.FC = () => (
    <div className="space-y-6">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="text-left py-3 px-4 text-sm font-medium text-gray-600">Téléphone</th>
                  <th className="text-left py-3 px-4 text-sm font-medium text-gray-600">Point de Vente</th>
                  <th className="text-right py-3 px-4 text-sm font-medium text-gray-600">Nombre d'Achats</th>
                ...
                {stats.topPurchaseVendors.map((vendor) => (
                  <tr key={vendor.vendor_id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="py-3 px-4 text-right">
                      <div className="flex items-center justify-end">
                        <span className={`text-sm font-medium ${
                          (vendor.purchase_to_sales_efficiency || 0) >= 80 ? 'text-green-600' :
                          (vendor.purchase_to_sales_efficiency || 0) >= 60 ? 'text-yellow-600' : 'text-red-600'
                        }`}>
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex items-center space-x-4">
            <span className="text-sm text-gray-500">
              {stats.purchaseDetails.length} achats au total
            </span>
            <button
            ...
            <div className="flex justify-between items-center mb-4">
              <div className="flex items-center space-x-4">
                <span className="text-sm text-gray-600">
                  Affichage de {startIndex + 1} à {Math.min(endIndex, stats.purchaseDetails.length)} sur {stats.purchaseDetails.length} achats
                </span>
                  ...
                  {currentPurchaseDetails.map((purchase) => (
                    <tr key={purchase.purchase_id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="py-3 px-4 text-right">
                        <div className="flex items-center justify-end">
                          <span className={`text-sm font-medium ${
                            (purchase.sales_efficiency || 0) >= 80 ? 'text-green-600' :
                            (purchase.sales_efficiency || 0) >= 60 ? 'text-yellow-600' : 'text-red-600'
                          }`}>
                      ...
                      <td className="py-3 px-4 text-sm text-gray-600">
                        {purchase.main_mobile_vendor ? (
                          <div>
                            ...
                            <p className="text-xs text-gray-500">Ratio: {(purchase.main_vendor_ratio || 0).toFixed(1)}%</p>
                          </div>
                        ) : (
      ...
      {/* Top achats par vendeur */}
      <div className="grid lg:grid-cols-2 gap-6">
        {stats.topPurchaseVendors.map((vendor) => (
          vendor.top_purchases && vendor.top_purchases.length > 0 && (
            <div key={vendor.vendor_id} className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
              <h4 className="text-md font-semibold text-gray-800 mb-4">
                Top Achats - {vendor.vendor_full_name}
              </h4>
              <div className="space-y-3">
                {vendor.top_purchases.slice(0, 5).map((purchase, index) => (
                  <div key={purchase.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-gray-50 transition-colors border border-gray-100">
                    <div className="flex items-center">
                      <div className="w-6 h-6 bg-gradient-to-r from-blue-500 to-blue-600 rounded-full flex items-center justify-center text-white text-xs font-medium shadow-md">
                        {index + 1}
                      </div>
                      <div className="ml-3">
